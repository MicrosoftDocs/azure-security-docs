...
```azurecli-interactive
az network vnet create -g "<resource-group>" -n "<management-vnet-name>" --address-prefixes "10.1.0.0/16" --tags "fastpathenabled=True" --subnet-name "<management-subnet-name>" --subnet-prefix "10.1.0.0/24"
```
...
```azurecli-interactive
az network vnet subnet update -g "<resource-group>" --vnet-name "<management-vnet-name>" -n "<management-subnet-name>" --delegations "Microsoft.HardwareSecurityModules/dedicatedHSMs"
```
...
```azurecli-interactive
az network vnet subnet show -g "<resource-group>" --vnet-name "<management-vnet-name>" -n "<management-subnet-name>"
```
...
Use the Azure PowerShell [New-AzDelegation](/powershell/module/az.network/new-azdelegation) cmdlet to create a service delegation to be added to your host subnet, and save the output to a variable:
...
$managementSubnetConfig = New-AzVirtualNetworkSubnetConfig -Name "<management-subnet-name>" -AddressPrefix $ManagementSubnetAddressPrefix -Delegation $delegation
```
...
```azurepowershell-interactive
New-AzVirtualNetwork -Name "<management-vnet-name>" -ResourceGroupName "<resource-group>" -Location "EastUS" -Tag $tags -AddressPrefix $ManagementVNetAddressPrefix -Subnet $managementSubnetConfig
```

To verify that the VNet was created correctly, use the Azure PowerShell [Get-AzVirtualNetwork](/powershell/module/az.network/get-azvirtualnetwork) cmdlet:

```azurepowershell-interactive
Get-AzVirtualNetwork -Name "<management-vnet-name>" -ResourceGroupName "<resource-group>"
```
...
```azurepowershell-interactive
New-AzDedicatedHsm -Name "<payment-hsm-name>" -ResourceGroupName "<resource-group>" -Location "East US" -Sku "payShield10K_LMK1_CPS60" -StampId "stamp1" -SubnetId "<subnet-id>" -ManagementSubnetId "<management-subnet-id>"
```
