...
1. For public-key operations such as encryption, wrapping, and verification, perform these operations locally without accessing Key Vault by caching the public key material. This approach not only reduces the risk of throttling but also improves resiliency of your application.
...
1. If still throttled wait 2 seconds, retry request
1. If still throttled wait 4 seconds, retry request
1. If still throttled wait 8 seconds, retry request
1. If still throttled wait 16 seconds, retry request
