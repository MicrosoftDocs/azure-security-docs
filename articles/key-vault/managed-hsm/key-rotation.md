...
```azurecli
az keyvault key rotation-policy update --hsm-name ContosoMHSM --name ContosoKey --value /path/to/policy.json
```
#### Example policies
...
### Azure CLI
```azurecli
az keyvault key rotate --hsm-name ContosoMHSM --name ContosoKey
```
