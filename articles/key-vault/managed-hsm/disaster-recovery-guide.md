...
The following example creates an HSM named **ContosoMHSM2**, in the resource group **myResourceGroup**, residing in the **West US 3** location, with **the current signed in user** as the only administrator.

```azurecli-interactive
oid=$(az ad signed-in-user show --query id -o tsv)
az keyvault create --hsm-name "ContosoMHSM2" --resource-group "myResourceGroup" --location "westus3" --administrators $oid
```
