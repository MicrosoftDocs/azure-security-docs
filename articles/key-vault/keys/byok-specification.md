...
**Key Exchange Key**: An HSM-backed key that customer generates in the key vault where the BYOK key is imported. This KEK must have following properties:

* It's an RSA-HSM key (4096-bit or 3072-bit or 2048-bit)
* It has a fixed key_ops (ONLY `import`), that allows it to be used ONLY during BYOK
* Must be in the same vault where the Target Key is imported
...
Use the **az keyvault key create** command to create KEK with key operations set to import. Note down the key identifier 'kid' returned from this command.
...
Customer uses HSM Vendor provided BYOK tool to create a key transfer blob (stored as a ".byok" file). KEK public key (as a .pem file) is one of the inputs to this tool.
...
The JSON blob is stored in a file with a ".byok" extension so that the Azure PowerShell or CLI client treats it correctly when ‘Add-AzKeyVaultKey' (PSH) or ‘az keyvault key import' (CLI) commands are used.

### Upload key transfer blob to import HSM-key

Customers transfer the Key Transfer Blob (".byok" file) to an online workstation and then run a **az keyvault key import** command to import this blob as a new HSM-backed key into Key Vault. 
