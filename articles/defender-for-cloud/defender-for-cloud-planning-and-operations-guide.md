...
description: This document helps you to plan before adopting Defender for Cloud and includes considerations regarding daily operations.
...
This guide is for information technology (IT) professionals, IT architects, information security analysts, and cloud administrators planning to use Defender for Cloud. Microsoft Learn hosts all documentation for Microsoft products and technologies.

## Planning guide

This guide provides the background for how Defender for Cloud fits into your organization's security requirements and cloud management model. It is important to understand how different individuals or teams in your organization use the service to meet secure development and operations, monitoring, governance, and incident response needs. The key areas to consider when planning to use Defender for Cloud are:
...
The next section describes recommendations for plannning how to handle each one of these areas and how to apply these recommendations based on your requirements.
...
Defender for Cloud uses [Azure role-based access control (Azure Role-based access control)](/azure/role-based-access-control/role-assignments-portal), which provides [built-in roles](/azure/role-based-access-control/built-in-roles) that can be assigned to users, groups, and services in Azure. When a user opens Defender for Cloud, they only see information related to resources to which they have access. This means that a user is assigned the role of Owner, Contributor, or Reader to the subscription or resource group to which a resource belongs. In addition to these roles, there are two roles specific to Defender for Cloud:

- **Security reader**: a user that is assigned to this role is only able to view Defender for Cloud configurations, including recommendations, alerts, policy, and health, but cannot make any changes.

- **Security admin**: includes the permissions assigned to the security reader, but the user can also update the security policy, and dismiss recommendations and alerts.
...
When planning access control using Azure Role-based access control for Defender for Cloud, make sure that you understand who in your organization needs access to Defender for Cloud to perform the tasks under their responsibility so that you can configure Azure Role-based access control properly.
...
|----------|-------------------|----------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------|
| Jeff     | Workload Owner    | Manages a cloud workload and its related resources. Responsible for implementing and maintaining protections in accordance with company security policy. | Resource Group Owner/Contributor.                                                                     |
| Ellen    | CISO/ISO          | Responsible for all aspects of security for the company. Understanding the company's security posture across cloud workloads. Needs to be informed of major attacks and risks. | Subscription Owner/Contributor or Security Admin.                                                     |
| David    | IT Security       | Sets company security policies to ensure the appropriate protections are in place. Monitors compliance with policies. Generates reports for leadership or auditors. | Subscription Owner/Contributor or Security Admin.                                                     |
| Judy     | Security Operations | Monitors and responds to security alerts at any time. Escalates to Cloud Workload Owner or IT Security Analyst. | Subscription Reader or Security Reader to view alerts. Subscription Owner/Contributor or Security Admin required to dismiss alerts. |
| Sam      | Security Analyst  | Investigates attacks. Works with Cloud Workload Owner to apply remediation.                               | Subscription Reader to view alerts. Subscription Owner/Contributor required to dismiss alerts. Access to the workspace might be required. |
...
> [!NOTE]
> Specifying a security contact ensures that Azure can reach the right person in your organization if a security incident occurs. Read [Provide security contact details in Defender for Cloud](configure-email-notifications.md) for more information on how to implement this recommendation.
...
- Verify that these policies are appropriate for your various subscriptions and resource groups.
...
In the Azure portal, you can browse to see a list of your Log Analytics workspaces, including any that Defender for Cloud created. A related resource group is created for new workspaces. Resources are created according to the following naming convention:

- Workspace: *DefaultWorkspace-[subscription-ID]-[geo]*

- Resource Group: *DefaultResourceGroup-[geo]*

For workspaces that Defender for Cloud created, data is retained for 30 days. For existing workspaces, retention is based on the workspace pricing tier. If you want, you can also use an existing workspace.

If your agent reports to a workspace other than the **default** workspace, any Defender for Cloud [Defender plans](defender-for-cloud-introduction.md#protect-cloud-workloads) that you enabled on the subscription, you also need to enable on the workspace. {DK: I wasn't clear if this was a recommendation to enable on the workspace, or if this is automatically enabled when you enable on the subscription}
...
The Defender for Cloud Overview provides a unified view of security across all your Azure resources and any non-Azure resources that you have connected. This example shows an environment with many issues to resolve:
...
> [!NOTE]
> Defender for Cloud does not interfere with normal operating procedures. Defender for Cloud passively monitors deployments and provides recommendations based on the security policies that your organization has enabled.

When initially opting in to use Defender for Cloud for your current Azure environment, ensure that you review all recommendations in the **Recommendations** page.

Plan to visit the threat intelligence option as part of your daily security operations. From there you can identify security threats against the environment, such as identifying whether a particular computer is part of a botnet.

### Monitor for new or changed resources

Most Azure environments are dynamic, with resources regularly being created, spun up or down {DK: What does it mean to spin resources up and down? Is that a standard industry term?}, reconfigured, and changed. Defender for Cloud helps ensure that you have visibility into the security state of these new resources.
...
As part of your security operations, you should also adopt preventative measures to restrict access to VMs, and control the applications that are running on VMs. By locking down inbound traffic to your Azure VMs, you are reducing exposure to attacks, and at the same time providing easy access to connect to VMs when needed. Use [just-in-time VM access](just-in-time-access-usage.yml) access feature to hardening access to your VMs.
...
Microsoft Azure Security Response in the Cloud lifecycle serves as the foundation for incident response stages. The stages of incident response in the cloud lifecycle are shown in the following diagram:
...
> [!NOTE]
> You can use the National Institute of Standards and Technology (NIST) [Computer Security Incident Handling Guide](https://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.800-61r2.pdf) as a reference to help you build your own incident response plan.
