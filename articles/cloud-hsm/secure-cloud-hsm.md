...
- **Verify user and key synchronization across all nodes**: Azure Cloud HSM operates as a cluster of three nodes. If user or key creation fails on one or more nodes, the user or key might exist on only some nodes. A user or key that exists on only one node is at risk of permanent, unrecoverable loss if that node fails. After creating any user or key, verify it exists on all nodes and synchronize any missing items immediately. For detailed steps, see [Synchronize users and keys across Azure Cloud HSM nodes](synchronize-users-keys.md).
