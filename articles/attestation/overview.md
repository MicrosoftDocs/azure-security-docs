...
Azure [Confidential Containers](/azure/confidential-computing/confidential-containers) is based on [AMD processors with SEV-SNP technology](/azure/confidential-computing/virtual-machine-options). Confidential containers, hosted on [Azure Container Instances](/azure/container-instances/container-instances-confidential-overview) and on [Azure Kubernetes Service (in preview)](/azure/aks/deploy-confidential-containers-default-policy), offer the ability to run groups of containers in an SEV-SNP protected trusted execution environment that isolates that group of containers from the container management control plane and other running containers. Attestation in confidential containers involves fetching the AMD hardware attestation report directly from the processor. You can accomplish this task with the [SKR sidecar container](https://github.com/microsoft/confidential-sidecar-containers/tree/main/cmd/skr) or compile it directly into your application logic. The hardware report can then be exchanged with Azure Attestation and [managed HSM](../key-vault/managed-hsm/overview.md) or Premium [Azure Key Vault (AKV)](../key-vault/general/basic-concepts.md) to retrieve secrets. You can also provide the hardware report to your own key vault system as desired.
...
1. **Verify if attestation token is generated by Azure Attestation** - An attestation token generated by Azure Attestation is signed using a self-signed certificate. The signing certificates URL is exposed via an [OpenID metadata endpoint](/rest/api/attestation/metadata-configuration/get?tabs=HTTP#get-openid-metadata). A relying party can retrieve the signing certificate and perform signature verification of the attestation token. For more information, see [code samples](https://github.com/Azure-Samples/microsoft-azure-attestation/blob/master/maa.jwt.verifier.dotnet/Program.cs#L94).
1. **Verify if Azure Attestation is running inside an SEV-SNP container** - The token signing certificates include information about the TEE inside which Azure Attestation runs. This TEE collateral is a SEV-SNP report. The relying party can check if Azure Attestation is running inside a valid TEE by locally validating the report. For code samples, see [SEV-SNP](https://github.com/Azure-Samples/microsoft-azure-attestation/blob/master/maa.jwt.verifier.dotnet/Program.cs#L63).
1. **Validate binding of Azure Attestation TEE report with the key that signed the attestation token** - The relying party can verify whether the hash of the public key that signed the attestation token matches the report data field of the Azure Attestation TEE report. For more information, see the [code sample](https://github.com/Azure-Samples/microsoft-azure-attestation/blob/master/maa.jwt.verifier.dotnet/Program.cs#L426).
1. **Validate if Azure Attestation code measurements match the Azure published values** - The TEE collateral embedded in the attestation token signing certificates includes TEE code measurements of Azure Attestation. A relying party can validate that the report belongs to Azure Attestation by comparing specific values retrieved from the TEE collateral in the attestation token signing certificate with values provided by the Azure Attestation team. For SEV-SNP, HOST_DATA should be validated. For more information, see the [code sample](https://github.com/Azure-Samples/microsoft-azure-attestation/blob/master/maa.jwt.verifier.dotnet/Program.cs#L395). If you're interested in performing this validation, submit a request on the [Azure support page](https://azure.microsoft.com/support/options/). The Azure Attestation team reaches out to you when these specific values are scheduled for rotation.
1. **Gain assurance in build provenance** - Azure Attestation is integrated with Microsoft's Signing Transparency (MST). MST records code signatures in an immutable, tamper-proof ledger. The MST service delivers cryptographically verifiable, [SCITT](https://datatracker.ietf.org/group/scitt/about/)-compliant receipts that provide clear visibility into Azure Attestation's deployments—strengthening trust, auditability, and regulatory compliance. To learn more about MST and how to validate build artifacts, see [About Microsoft Signing Transparency ledger](../confidential-ledger/about-microsoft-signing-transparency-ledger.md).

The values identifying valid Azure Attestation instances are expected to change when code-signing certificates are rotated or when security updates require new policy versions. The Azure Attestation team follows this rollout schedule for every planned rotation:
...
1. After the two-month grace period, Azure Attestation starts using the new values.
1. Three months after the notification date, Azure Attestation stops using the old values.
...
- The [Azure Traffic Manager](/azure/traffic-manager/) for the region detects that the health probe is degraded and switches the endpoint to the paired region.
- Existing connections don't work and receive internal server error or timeout issues.
- All control plane operations are blocked. Customers can't create attestation providers in the primary region.
- All data plane operations, including attest calls and policy configuration, are served by the secondary region. Customers can continue to work on data plane operations with the original URI corresponding to the primary region.
