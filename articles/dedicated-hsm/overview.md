...
> [!IMPORTANT]
> We're pleased to announce that [Azure Cloud HSM](../dedicated-hsm/overview.md) is now generally available and is the successor to Azure Dedicated HSM. As part of this transition, Microsoft is**no longer onboarding new customers to Dedicated HSM**. New customers are encouraged to evaluate and onboard to [Azure Cloud HSM](../dedicated-hsm/overview.md), [Azure Managed HSM](../key-vault/managed-hsm/overview.md), or [Azure Key Vault]((../key-vault/general/overview.md) based on their specific workload requirements; for guidance, see [How to choose the right Azure key management solution](/azure/security/fundamentals/key-management-choose).
