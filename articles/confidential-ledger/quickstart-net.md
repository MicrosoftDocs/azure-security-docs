...
In this quickstart, the logged in user is used to authenticate to Azure confidential ledger, which is the preferred method for local development. The name of your confidential ledger is expanded to the ledger URI, in the format `https://<ledger-name>.confidential-ledger.azure.com`. This example uses the ['DefaultAzureCredential()'](/dotnet/api/azure.identity.defaultazurecredential) class from [Azure Identity Library](/dotnet/api/overview/azure/identity-readme), which allows you to use the same code across different environments with different options to provide identity. 
