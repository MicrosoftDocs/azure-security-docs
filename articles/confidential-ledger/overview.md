...
[For Azure Blob Storage](/azure/confidential-ledger/create-blob-managed-app), confidential ledger enhances security features by providing an immutable log of storage operations. This log is valuable for regulatory compliance and archival purposes where data integrity over time is crucial.
...
Administrators can create and manage confidential ledger with administrative APIs (control plane) for example, delete a resource or move it across resource groups. Confidential ledger provides functional APIs (data plane) for data CRUD operations, such as CREATE, UPDATE, PUT, and GET.
...
- Azure confidential ledger names must be globally unique. Ledgers with the same name, no matter their type, aren't allowed.
